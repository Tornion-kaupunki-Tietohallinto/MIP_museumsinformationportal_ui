angular.module('mip.directives').directive('mipDialog', [ 'ModalService', '$window', 'ModalControllerService', function(ModalService, $window, ModalControllerService) {
   return {
       restrict : 'A',
       link : function(scope, element, attrs) {
           ...
           element.bind('click', function(event, isHide) {
               ...
               var chromeWayClose = 'fa fa-window-close';
               ...
               if(event.target.tagName !== "A" && event.target.tagName !== "TD"){
                    ...
                    element.addClass("active-modal");
                    element.css('z-index', ModalService.getNextModalZIndex());
                    // Päivitetään pikanäppäimet oikeaan ikkunaan, jos liikutaan niiden välillä
                    ModalControllerService.bindHotkeys(scope);
               }

               // Piilota klikattu eli pienennys-kuvake tai sulje klikattu eli rasti-kuvake
               if(clickEventTarget === ie_firefoxWayTarget || clickEventTarget === chromeWayTarget || clickEventTarget === chromeWayClose){
                    element.removeClass('active-modal');
                    element.css('display', 'none');
                    // Poistetaan pikanäppäimet
            ...
            //Lisätään pikanäppäimet, kun modal avataan
            ModalControllerService.bindHotkeys(scope);
