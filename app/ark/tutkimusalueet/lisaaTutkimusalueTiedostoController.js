...
/**
 * Tutkimusalueen tiedoston lisäysControllers
 *
 */
angular.module('mip.file').controller(
        'LisaaTutkimusalueTiedostoController',
        [
                '$scope', '$rootScope', 'CONFIG', '$http', 'ModalService', 'AlertService', '$timeout', 'FileService',
                'Upload', 'tutkimus', 'UserService', 'locale', '$window', 'selectedModalNameId',
                'ModalControllerService', 'parentModalId', 'tutkimusalue',
                function($scope, $rootScope, CONFIG, $http, ModalService, AlertService, $timeout, FileService,
                    ...
                    vm.upload = function() {
                        ...
                        if(vm.files.length === 1) {
                            FileService.saveArkFile(vm.files[0], {}, CONFIG.ENTITY_TYPE_IDS['tutkimusalue'], vm.tutkimus.properties.id, 'lisaa_tutkimusalue').then(function success(response) {
                            ...
                            FileService.saveArkFile(vm.files, {}, CONFIG.ENTITY_TYPE_IDS['tutkimusalue'], vm.tutkimus.properties.id, 'lisaa_tutkimusalue').then(function success(response) {
                                var data = response;
