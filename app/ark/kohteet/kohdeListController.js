...
angular.module('mip.kohde').controller(
    'KohdeListController',
    [
        '$scope', 'TabService', '$location', 'TutkimusService', '$filter', 'NgTableParams', 'CONFIG', 'KuntaService', 'ModalService', 'AlertService',
        ...
        'ListControllerService', '$q',
        function ($scope, TabService, $location, TutkimusService, $filter, NgTableParams, CONFIG, KuntaService, ModalService, AlertService,
            ListService, locale, Auth, $rootScope, MapService, CacheFactory, EntityBrowserService, InventointiprojektiService, UserService, KohdeService,
            ListControllerService, $q) {
            ...
            vm.kohteetTable = new NgTableParams({
                ...
                getData: function ($defer, params) {
                    var promises = [
                        Auth.checkPermissions('arkeologia', 'ark_kohde'),
                        TutkimusService.getAktiivisetInventointitutkimukset()
                    ];

                    $q.all(promises).then(function success(data) {
                        var permissions = data[0];
                        var aktiivisetInventointitutkimuksetLength = data[1].features.length;

                        if (permissions.luonti || aktiivisetInventointitutkimuksetLength > 0) {
