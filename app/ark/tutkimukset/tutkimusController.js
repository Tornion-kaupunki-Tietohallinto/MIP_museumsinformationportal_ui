...
angular.module('mip.tutkimus').controller(
		...
		[
				...
				'UserService', 'NgTableParams', 'YksikkoService', '$popover', 'permissions',
				 ...
				 'RaporttiService', 'FileService', 'KarttaService', 'KohdeService',
				function ($scope, $rootScope, TabService, $location, $filter, TutkimusService,
				    ...
				    vm.setUp = function() {
	                    ...
	                    // 1) Pääkäyttäjä
	                    // 2) Tutkija
	                    // 3) Katselija JA tutkimusta ei ole merkitty valmiiksi.
                     ...
                     */
                    vm.avaaKohde = function(){
                    	if(vm.tutkimus.properties.kohde){
                    		var kohde = vm.tutkimus.properties.kohde;
                            KohdeService.fetchKohde(kohde.id).then(function(haettu_kohde) {
                        ...
                        if (!vm.edit) {
                            ...
                            if (typeof featureHit !== 'undefined') {
								...
								if(layerHit.getProperties().name == 'Kohteet'){
									// Käytetään vm.ladattuKohde, koska kohten sijainnista johon osutaan ei saada kohteen idä.
									KohdeService.fetchKohde(vm.ladattuKohde.properties.id).then(function(kohde){
