...
angular.module('mip.arvoalue').controller(
        ...
        [
                ...
                'MuutoshistoriaService', 'EntityBrowserService', 'selectedModalNameId', 'RakennusService', 'LocationService',
                function($scope, $q, $location, TabService, CONFIG, existing, $http, ModalService,
                    ...
                    $scope.selectLayer = function() {
                        ...
                        var reititLayer = null;
                        for(var i = 0; i<$scope.mapLayers.length; i++) {
                            if($scope.mapLayers[i].name == 'Reitit') {
                                reititLayer = $scope.mapLayers[i];
                            }
                        }
                        $scope.updateLayerData('Reitit');
                    ...
                    $scope.updateLayerData = function(layerName) {
                        var l = null;
                         ...
                         if(l && l.active) {
                             ...
                             if(l.name == 'Reitit') {
                                LocationService.getReitit({'entiteettiTyyppi': 'Arvoalue', 'entiteettiId': $scope.arvoalue.properties.id}).then(function success(data) {
                                    ...
                                    if (layerHit.getProperties().name == 'Reitit') {
                                        ...
                                        for(var i = 0; i < $scope.reitit.length; i++) {
                                            if($scope.reitit[i].properties.id === featureId) {
                                                ModalService.reittiModal($scope.reitit[i], 'rakennusinventointi', 'arvoalue', $scope.arvoalue);
                                                break;
                                            }
                                        }
